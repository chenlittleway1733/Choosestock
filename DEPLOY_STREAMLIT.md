...
- `ui_main.py`
- `services.py`（FY1／FY2／FY3 FactSet 程式備援）
...
若 TTM EPS 有值但 FY1／FY2／FY3 全部為空，請確認根目錄的 `services.py` 與 `ui_panels/financials.py` 都已覆蓋。新版會先查 yfinance，再以 FactSet／鉅亨網明確年度表補齊；仍查無資料時，畫面會依金鑰狀態清楚提示載入 `key.txt` 並按右上方 AI 補缺按鈕。
