...
# 🛡️ 徹底還原早上版的穩定爬蟲邏輯 (包含 KY 股字眼相容與 div 容錯)
...
if curr_id:
    ...
    if hist is not None and not hist.empty:
        ...
        if pe_ratio is None and df_per_backup is not None and not df_per_backup.empty:
            if (pd.Timestamp.today() - df_per_backup.iloc[-1]['date']).days < 30:
        ...
        # 預先計算所有要顯示的字串，徹底杜絕 SyntaxError 發生
        if use_custom_eps:
        ...
        if use_custom_eps:
            f_eps_display = f"{t_eps_str} / <span style='color:#00bfff;'>{f_eps_str}</span>"
        else:
            f_eps_display = f"{t_eps_str} / {f_eps_str}"
        ...
        # 🚀 修正過的 fund_html (無 inline if-else)
        fund_html = f"""
